...
`s` **security** — open Dependabot advisories across the sites registry, read from `sites cves` rather than scanned out of GitHub. The registry groups per advisory (the same CVE in fourteen sites is one row and one fix, not fourteen) and, more usefully, already knows which **tier** each vulnerable manifest sits in:
  ...
  Every `sites` call this tool makes sets `SITES_NO_AUTO_UPDATE=1`: the CLI self-updates silently, and a dashboard polling in the background has no business swapping a shared binary — or invalidating a live session's token — under whoever is using it interactively. Your own invocations still update it normally.
